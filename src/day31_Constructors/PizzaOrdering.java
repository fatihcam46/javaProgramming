...
public class PizzaOrdering {
    public static void main(String[] args) {
        ...
        ArrayList<Pizza> pizzas = new ArrayList<>();//it should be arraylist thats dinamic
        //looop we need 100 times
        for (int i = 0; i < 100; i++) {

            Pizza smallPizza = new Pizza('S', 2, 3);
            Pizza mediumPizza = new Pizza('M', 3, 4);
            Pizza largePizza = new Pizza('L', 4, 5);
//addAll methods use >>>> small+medium+large
            pizzas.addAll(Arrays.asList(smallPizza, mediumPizza, largePizza) );

        }
        System.out.println("Total number of pizza: "+pizzas.size());//size used>>Total number of pizza: 300

        double totalPrice = 0;//first 0
//for each loop>>>
