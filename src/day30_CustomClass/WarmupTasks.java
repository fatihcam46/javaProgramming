...
public class WarmupTasks {
    public static void main(String[] args) {
        ...
        Collections.swap(numbers, 0,  numbers.size()-1);//swap:first element(0index) last elemnt
...
//1. write a program that can swap the first and last elements of an ArrayList
  ...
  /*      2. Write a program that can move all the zeros to the last indexes of ArrayList
        ...
        int size = list.size();
        list.removeAll( Arrays.asList(0)  );//remove all zero`s
...
//loop we need:
        ...
        System.out.println("-------------------warm up 2 for each loop use::-------------------------");
        ...
        for (Integer each : list2) { //add zero`s
            if(each == 0){
                result.add(each);
            }        }
...
//use for looop
        ...
        //use Lambda , find just letters:  removeIf method:
...
//letters = [A, B, C, D, E, F, G]
        ...
        //2nd way: remove letters and digits:
...
/* 3. Write a program that can extract the special characters, digits and letters from
 a string and stores them into separate ArrayLists of Characters
 			Ex:		str = "ABCD123$%#@&456EFG!"
			output:
					list1: {1, 2, 3, 4, 5, 6}
					list2: {A, B, C, D, E, F, G}
